#include "elite_bosses.h"

struct Elite_Bosses bosses[] =
    {
        {"Crocodile",
         10,
         200,
         1000,
         500,
         200,
         0,
         {
             {"Desert Spada", 1.5},
             {"Sables", 2.0},
             {"Ground Death", 2.5},
         }},

        {"Enel",
         20,
         500,
         2000,
         500,
         200,
         0,
         {
             {"El Thor", 2.0},
             {"Mamaragan", 2.5},
             {"Raigo", 3.0},
         }},

        {"Rob Lucci: Ennies Lobby",
         30,
         1000,
         3000,
         500,
         200,
         0,
         {
             {"Shigan Madara", 2.0},
             {"Rankyaku Amane Dachi", 2.5},
             {"Rakuogan", 3.0},
         }},

        {"Gecko Moria",
         35,
         1500,
         4000,
         500,
         200,
         0,
         {
             {"Brick Bat", 2.0},
             {"Shadow Revolution", 2.2},
             {"Tsuno-Tokage", 2.5},
         }},

        {"Kizaru: Sabaody",
         40,
         1500,
         4500,
         500,
         200,
         0,

         {
             {"Yasakani no Magatama", 2.2},
             {"Yata no Kagami", 1.0},
             {"Ama No Murakumo", 3.0},
         }},

        {"Megellan",
         45,
         1600,
         5000,
         500,
         200,
         0,
         {
             {"Hydra", 2.0},
             {"Chloro Ball", 2.2},
             {"Venom Demon: Hell's Judgment", 4.0},
         }},

        {"Kizaru: MarineFord",
         50,
         1800,
         5500,
         500,
         200,
         0,
         {
             {"Yasakani no Magatama", 2.0},
             {"Ama no Murakumo", 1.0},
             {"Yata no Kagami", 3.0},
         }},

        {"Kuzan: Marineford",
         55,
         2000,
         6000,
         500,
         200,
         0,
         {
             {"Ice Age", 2.0},
             {"Ice Block: Partisan", 2.2},
             {"Ice Ball", 2.5},
         }},

        {"Akainu: Marineford",
         60,
         2500,
         4000,
         500,
         200,
         0,
         {
             {"Dai Funka", 2.0},
             {"Ryusei Kazan", 2.5},
             {"Meigo", 3.0},
         }},

        {"Donquixote Doflamingo",
         65,
         2200,
         7000,
         500,
         200,
         0,
         {
             {"Parasite", 1.5},
             {"Overheat", 2.2},
             {"God Thread", 3.0},
         }},

        {"Big Mom: Whole Cake Island",
         70,
         2400,
         7500,
         500,
         200,
         0,
         {
             {"Ikoku", 2.5},
             {"Cognac", 3.0},
             {"Soul Pocus", 1.5},
         }},

        {"Kaido",
         80,
         2500,
         8000,
         500,
         200,
         0,
         {
             {"Thunder Bagua", 2.0},
             {"Kongo Kabura", 2.5},
             {"Shuron Hakke", 1.0},
         }},

        {"Big Mom: Wano",
         85,
         2600,
         12000,
         500,
         200,
         0,
         {
             {"Indra", 2.0},
             {"Maser Ho", 2.5},
             {"Ikoku Sovereignty", 3.0},
         }},

        {"Kaido: Hybrid",
         88,
         2700,
         10000,
         500,
         200,
         0,
         {
             {"Thunder Bagua", 2.2},
             {"Ragnaraku", 2.0},
             {"Kundali Dragon Swarm", 3.0},
         }},

        {"Kaido: Dragon",
         90,
         3200,
         7000,
         500,
         200,
         0,
         {
             {"Bolo Breath", 2.0},
             {"Kaifu", 2.2},
             {"Dragon Twister", 2.5},
         }},

        {"Kizaru: Egghead",
         95,
         2800,
         7500,
         500,
         200,
         0,
         {
             {"Yasakani no Magatama", 1.8},
             {"Ama no Murakumo", 2.4},
             {"Yata no Kagami", 2.8},
         }},

        {"Saint Jaygarcia Saturn",
         100,
         3000,
         16000,
         500,
         200,
         0,
         {
             {"Venomous Legs", 3.0},
             {"Eye Paralysis", 1.0},
             {"Venomous Spit", 2.5},
         }},

        {"Saint Marcus Mars",
         105,
         3100,
         12000,
         500,
         200,
         0,
         {
             {"Itsumade Charge", 3.0},
             {"Beak Attack", 2.5},
             {"Conqueror's Haki", 2.0},
         }},

        {"Saint Topman Warcury",
         110,
         3100,
         14000,
         500,
         200,
         0,
         {
             {"Fengxi Charge", 2.2},
             {"Tusk Slam", 3.2},
             {"Conqueror's Haki Roar", 2.0},
         }},

        {"Saint Ethanbaron V. Nusjuro",
         115,
         3600,
         11000,
         500,
         200,
         0,
         {
             {"Ice Blade Slash", 3.0},
             {"Bakotsu Charge", 2.4},
             {"Frozen Slash", 2.0},
         }},

        {"Saint Shepherd Ju Peter",
         120,
         3000,
         13000,
         500,
         200,
         0,
         {
             {"Sandworm Devour", 2.2},
             {"Vacuum Inhalation", 1.8},
             {"Underground Ambush", 2.0},
         }},

        {"Saint Gunko",
         125,
         2800,
         9500,
         500,
         200,
         0,
         {
             {"Arrows", 1.5},
             {"Elastic Limbs", 1.2},
             {"Arrow Barrage", 1.8},
         }},

        {"Saint Gunko: Imu",
         130,
         4200,
         16000,
         500,
         200,
         0,
         {
             {"Supreme King Haki Burst", 3.0},
             {"Domi Reversi", 1.0},
             {"Demonic Pistol", 2.8},
         }},

        {"Saint Shepherd Sommers",
         135,
         3400,
         13500,
         500,
         200,
         0,
         {
             {"Thorn Manipulation", 1.8},
             {"Fear Manifestation", 1.4},
             {"Thorn Bind", 1.0},
         }},

        {"Saint Rimoshifu Killingham",
         138,
         3300,
         13000,
         500,
         200,
         0,
         {
             {"Sleep Inducement", 1.0},
             {"Nightmare Manifestation",1.2},
             {"MMA Summoning",1.4},
         }},

        {"Imu: King of the World",
         150,
         50000,
         1000000,
         500000,
         20000,
         0,
         {
             {"Supreme King Haki", 20.0},
             {"Domi Reversi", 10.0},
             {"Maki", 30.0},
         }},
};

int boss_count = 26;
